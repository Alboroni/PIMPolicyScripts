            ...
            setting = @{
                ...
                approvalStages = @(
                    @{
                        ...
                        primaryApprovers = @(
                          #   @{
                               #  id = "8b62a62d-1ada-4ba8-9088-7a03d05c791e"
                                #description = "Alex1" # This Is The Display Name Of The Approver Which Appears In Lowercase Text In The Portal When Using The API - When Configuring Via The Portal it Correctly Resolves To The AAD Display Name & UPN
                                 #isBackup = "false"
                             #    userType = "User" # "User"
                             #}
                        )
...
############### List Existing Eligible Role Assignments Starts ###############
    ...
    If ($exportFiles -eq $true) {
        ...
        $outputEligibleRoleAssignmentsFileNameBeforeChange = $subscriptionName + "-EligibleRoleAssignmentsBefore.json"
...
############### Grant The Eligible Role Assignment Starts ###############
    ...
    If ($deleteAssignmentOnly -eq $false) {
        ...
        If ($exportFiles -eq $true) {
            ...
            $outputEligibleRoleAssignmentsFileNameAfterChange = $subscriptionName + "-EligibleRoleAssignmentsAfter.json"
